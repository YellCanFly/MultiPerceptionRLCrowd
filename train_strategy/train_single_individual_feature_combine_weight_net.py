...
from model.ppo_raycast_nav_feature_combine_weight_mlp_net import PPO
...
def main():
    ...
    if args.pretrained_entity_model_path != '':
        for rl_model in rl_models.values():
            rl_model.load_models_entity(args.pretrained_entity_model_path)

    if args.pretrained_implicit_model_path != '':
        for rl_model in rl_models.values():
            rl_model.load_models_implicit(args.pretrained_entity_model_path)
    ...
    for episode_id in range(args.n_episodes):
        ...
        # 根据weight_random_range随机生成weight_collision和weight_terrain,
        # episode_id在args.n_episodes内的位置决定了weight_random_range的取值索引
        weight_id = episode_id // (args.n_episodes // len(weight_random_range))
        weight_id = min(weight_id, len(weight_random_range) - 1)
        ...
        vec_states, img_states = env.reset(width=env_size[0], height=env_size[1],
                                           ...
                                           reward_weight_collision_range=[weight_random_range[weight_id][0][0],
                                                                          weight_random_range[weight_id][0][1]],
                                           reward_weight_terrain_range=[weight_random_range[weight_id][1][0],
                                                                        weight_random_range[weight_id][1][1]]
                                           )
        ...
        while not env.all_agent_done:
            ...
            if args.model_name == 'ddpg' or args.model_name == 'td3':
                ...
                if episode_step != 0 and episode_step % args.delay_learn_step == 0:
        ...
        if update_agent_score_avg >= max(recent_avg_scores):
            print('saving ascent: episode_id=', episode_id, 'update_agent_score=%.2f' % update_agent_score,
                  'update_agent_score_avg=%.2f' % update_agent_score_avg)
