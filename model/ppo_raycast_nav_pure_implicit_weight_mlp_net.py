...
class ActorNetwork(nn.Module):
    ...
    def forward(self, state_vec, state_img):
        ...
        img_feature = img_feature.reshape(agent_state.size()[:-1] + img_feature.size()[-1:])
        weight_feature = self.model_weight(weight_state[..., 1].unsqueeze(-1))
        implicit_feature = self.model_implicit(
            torch.cat((img_feature, agent_state, weight_feature), dim=-1))

        action_mean = self.model_mu(implicit_feature)
    ...
    def save_implicit(self, save_path=None):
        """
        ...
        if save_path is None:
            ...
            chkpt_dir = save_path
        print(f"Saving implicit model to {chkpt_dir}")
        chkpt_file_implicit = os.path.join(chkpt_dir, self.name + '_implicit')
        torch.save(self.model_implicit.state_dict(), chkpt_file_implicit)
        chkpt_file_img = os.path.join(chkpt_dir, self.name + '_img')
        torch.save(self.model_img.state_dict(), chkpt_file_img)
        chkpt_file_implicit_weight = os.path.join(chkpt_dir, self.name + '_implicit_weight')
        torch.save(self.model_weight.state_dict(), chkpt_file_implicit_weight)
...
class CriticNetwork(nn.Module):
    ...
    def forward(self, state_vec, state_img):
        ...
        return self.critic(implicit_feature)
...
class PPO(RLModel):
    ...
    def save_models_implicit(self, save_path=None):
        self.actor.save_implicit(save_path)
        self.critic.save_implicit(save_path)
...
if __name__ == '__main__':
    ...
    pretrained_pure_implicit_model_path = '../checkpoint_ppo/2024_12_18_7_55_39/up_to_date'
    rl_model.load_models(pretrained_pure_implicit_model_path)
    rl_model.save_models_implicit(pretrained_pure_implicit_model_path)
