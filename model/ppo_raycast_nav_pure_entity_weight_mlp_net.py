...
class PPOMemory:
    ...
    def store_memory(self, state_vec, action, probs, val, reward, done):
...
class PPOMemories:
    ...
    def get_all_memories(self):
        # 初始化存储所有智能体样本的列表
        state_vecs, actions, probs, vals, rewards, dones = [], [], [], [], [], []
...
class ActorNetwork(nn.Module):
    def __init__(self, lr, ray_vec_dims, agent_vec_dims, weight_vec_dims, state_img_dims, action_dims,
        ...
        self.model_weight = nn.Sequential(
        ...
        init_net(self.model_weight[0])
        init_net(self.model_weight[3])
        ...
        self.model_mu = nn.Sequential(
            nn.Linear(self.fc2_dims, self.action_dims),
            nn.Tanh()
        )
    ...
    def forward(self, state_vec, state_img):
        ...
        weight_feature = self.model_weight(weight_state[..., 0].unsqueeze(-1))

        # print(torch.cat((ray_state, agent_state, weight_state[..., 0].unsqueeze(-1)), dim=-1).shape)
        entity_feature = self.model_entity(
            torch.cat([ray_state, agent_state, weight_feature], dim=-1))

        action_mean = self.model_mu(entity_feature)
    ...
    def save_entity(self, save_path=None):
        """
        Save the parameters of model_entity to the checkpoint file.
        """
        if save_path is None:
            chkpt_dir = self.chkpt_dir
        else:
            chkpt_dir = save_path
        chkpt_file = os.path.join(chkpt_dir, self.name+'_entity')
        torch.save(self.model_entity.state_dict(), chkpt_file)
        chkpt_file_weight = os.path.join(chkpt_dir, self.name + '_entity_weight')
        torch.save(self.model_weight.state_dict(), chkpt_file_weight)
        print(f"Saving model to {chkpt_file}")
...
class CriticNetwork(nn.Module):
    ...
    def forward(self, state_vec, state_img):
        ...
        return self.critic(entity_feature)
...
class PPO(RLModel):
    ...
    def remember(self, agent_id, index, state_vec, action, reward, done):
        ...
        self.memories.store_memory(agent_id, state_vec, action, probs, val, reward, done)
    ...
    def save_models_entity(self, save_path=None):
        self.actor.save_entity(save_path)
        self.critic.save_entity(save_path)
    ...
    def learn(self):
        ...
        action_arr = memory[1]  # agent_num * max_mem * action_dims
        old_probs_arr = memory[2]  # agent_num * max_mem
        vals_arr = memory[3]  # agent_num * max_mem
        reward_arr = memory[4]  # agent_num * max_mem
        done_arr = np.array(memory[5], dtype=int)  # agent_num * max_mem

        print(state_vec_arr.shape)
        ...
        for _ in range(self.n_epochs):
            ...
            for batch in batches:
                ...
                # agent_num * batch_size * action_dims
                dist = self.actor(state_vecs, None)

                # agent_num * batch_size
                critic_value = self.critic(state_vecs, None)
...
if __name__ == '__main__':
    ...
    pretrained_pure_entity_model_path = '../checkpoint_ppo/2024_12_9_16_47_59/up_to_date'
    rl_model.load_models(pretrained_pure_entity_model_path)
    rl_model.save_models_entity(pretrained_pure_entity_model_path)